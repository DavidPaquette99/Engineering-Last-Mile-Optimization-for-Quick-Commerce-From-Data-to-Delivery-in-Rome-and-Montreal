...
from Simulation_Engine_Haversine import simulate_day, calculate_payment
import random
import copy
import re
...
assignment_modes = ["naive", "hungarian", "anfis", "pairwise"]
random.seed(42)
...
for day_offset in range(num_days):
    ...
    for assignment_mode in assignment_modes:
        ...
        pd.DataFrame(orders_data).to_csv(
            f"{output_dir}/{city.lower()}_orders_completed_{simulated_date.date()}_{assignment_mode}_final.csv",
            index=False
        )
        ...
        pd.DataFrame(couriers_data).to_csv(
            f"{output_dir}/{city.lower()}_couriers_completed_{simulated_date.date()}_{assignment_mode}_final.csv",
            index=False
        )
    ...
    pd.DataFrame(daily_summary_rows).to_csv(
        f"{output_dir}/{city.lower()}_haversine_summary_completed_{simulated_date.date()}_final.csv",
        index=False
    )
...
summary_files = [
    ...
    if re.match(f"{city.lower()}_haversine_summary_completed_\\d{{4}}-\\d{{2}}-\\d{{2}}\\_final.csv", f)
]
...
full_summary.to_csv(
    os.path.join(output_dir, f"{city.lower()}_haversine_full_summary_completed_final.csv"),
    index=False
)
print(f"Full summary saved to {output_dir}/{city.lower()}_haversine_full_summary_completed_final.csv")