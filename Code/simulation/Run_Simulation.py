...
assignment_modes = ["naive", "hungarian", "anfis", "pairwise"] #"naive", "hungarian", "anfis", 
random.seed(42)
...
for day_offset in range(num_days):
    ...
    for assignment_mode in assignment_modes:
        ...
        pd.DataFrame(orders_data).to_csv(
            f"{output_dir}/{city.lower()}_orders_{simulated_date.date()}_{assignment_mode}.csv",
            index=False
        )
        ...
        pd.DataFrame(couriers_data).to_csv(
            f"{output_dir}/{city.lower()}_couriers_{simulated_date.date()}_{assignment_mode}.csv",
            index=False
        )
    ...
    pd.DataFrame(daily_summary_rows).to_csv(
        f"{output_dir}/{city.lower()}_summary_{simulated_date.date()}.csv",
        index=False
    )
...
summary_files = [
    ...
    if re.match(f"{city.lower()}_summary_\\d{{4}}-\\d{{2}}-\\d{{2}}\\.csv", f)
]
...
full_summary.to_csv(
    os.path.join(output_dir, f"{city.lower()}_full_summary.csv"),
    index=False
)
print(f"Full summary saved to {output_dir}/{city.lower()}_full_summary.csv")
