...
def load_anfis_regression_model(city, base_path):
    model_path = os.path.join(base_path, f"{city}_anfis_model_haversine.pkl")
    with open(model_path, "rb") as f:
        model = pickle.load(f)
    scaler_X = joblib.load(os.path.join(base_path, f"{city}_scaler_X_haversine.pkl"))
    scaler_y = joblib.load(os.path.join(base_path, f"{city}_scaler_y_haversine.pkl"))
...
def anfis_assignment(
        ...
        model, scaler_X, scaler_y, all_orders):
    """
    ...
    for order in list(new_orders):
        ...
        # actual times/distances (using haversine)
