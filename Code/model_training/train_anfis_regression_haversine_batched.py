...
for city in CITIES:
    ...
    # Load new batch-aware haversine-based dataset
    df = pd.read_csv(os.path.join(DATA_PATH, f"{city}_regression_training_data_haversine_batch.csv"))
    ...
    if hasattr(model, 'loss_train'):
        ...
        plt.tight_layout()
        plt.savefig(f"{DATA_PATH}/{city}_anfis_training_curve_haversine.png", dpi=200)
        plt.show()
    else:
    ...
    model.save_model(save_path=DATA_PATH, filename=f"{city}_anfis_model_haversine_batched.pkl")
    joblib.dump(scaler_X, os.path.join(DATA_PATH, f"{city}_scaler_X_haversine_batched.pkl"))
    joblib.dump(scaler_y, os.path.join(DATA_PATH, f"{city}_scaler_y_haversine_batched.pkl"))
